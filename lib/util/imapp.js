...
App.prototype.fingerprintstore = function () {
	var settings = _.findWhere(this._platforms, {
		platform: process.platform
	});
	if (!settings) return;
	return resolve_home_path(settings.fingerprints);
};

App.prototype.instagstore = function () {
	var settings = _.findWhere(this._platforms, {
		platform: process.platform
	});
	if (!settings) return;
	return resolve_home_path(settings.instags);
};
	...
	if (fs_existsSync(filename)) {
		...
		if (buddies && buddies.length) {
			buddies.forEach(function (line) {
				...
				if (entry[4]) self._parsed_fingerprints.push({
					username: entry[0],
					accountname: entry[1],
					protocol: entry[2],
					fingerprint: entry[3]
...
App.prototype.match = function (fp) {
	...
	if (this._parsed_fingerprints.length) {
		this._parsed_fingerprints.forEach(function (entry) {
			if (fingerprints.equal(entry.fingerprint, fp)) match = entry;
		});
	}
	return match;
};
