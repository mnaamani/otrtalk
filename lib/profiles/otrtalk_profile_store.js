...
var OTRTALK_ROOT = path.join(USER_HOME, ".otrtalk"); //root directory of all otrtalk profile files
var CONFIG_PATH = path.join(OTRTALK_ROOT, "id.json"); //stores profile information (id, and buddies)
...
function FileStore() {
	...
	self.bindToProfile = function (name) {
		...
		api.pathToKeys = function () {
			return fullyQualifiedPath("./" + name + "/priv.keys");
		};
		api.pathToFingerprints = function () {
			return fullyQualifiedPath("./" + name + "/fingerprints/");
		};
		api.pathToInstags = function () {
			return fullyQualifiedPath("./" + name + "/instance.tags");
		};
...
/*
  ...
  profile information is saved in $(HOME)/.otrtalk/id.json
*/
