...
Profiles.prototype.count = function(){
  return this.profiles().length;
}
Profiles.prototype.empty = function(){
  return (this.count() ? false : true);
}
Profiles.prototype.multiple = function(){
  return (this.count()>1 ? true : false);
}
Profiles.prototype.firstProfileName = function(){
  if(this.count()){
    return this.profiles()[0];
  } return undefined;
}
Profiles.prototype.exists = function(lookup){
  var exists = false;
  if(this._cache.profiles){
    Object.keys(this._cache.profiles).forEach(function(name){
...
Profiles.prototype.profile = function(name,dont_load_otr){
    var data = this._cache.profiles[name];
    if(!data) return undefined;
    var profile = new Profile(name,data,dont_load_otr);
...
Profiles.prototype.add = function(name,data){
...
Profiles.prototype.remove = function(name){
    ...
    var profile = this.profile(name,true);
    if(profile){
...
function Profile(name,config,dont_load_otr){
  this._name = name;
  this._config = config;
  if(!dont_load_otr) this._otr = require(config.otr);
}
...
Profile.prototype.save = function(){
  ...
  if(this._userFiles) this._userFiles.save();
}
...
Profile.prototype.openKeyStore = function (buddy,password){
  ...
  var otrm = this.OTR();
  this._user = new otrm.User(this._userFiles);
  return this;
}
...
Profile.prototype.printAccounts = function(){
  var user = this._user;
  var Table = require("cli-table");
  var table = new Table({
...
Profile.prototype.generateKey = function(password,next){
    var profile = this;
...
Profile.prototype.importKey = function(privkey,password){
  try{
    this.openKeyStore(undefined,password);
    this._user.importKey(this.name(),"otrtalk",privkey.export());
    this.print();
    this.printAccounts();
    this.save();
    console.log("Imported key successfully to profile:",this.name());
    return;//success
  }catch(E){
