...
Profiles.prototype.add = function(name,data,overwrite,dont_save){
    if(!name.match( /^[A-Z0-9]+$/ig)){
        console.log("Invalid profile name, use only alphanumerical characters.");
        return undefined;
    }
    if(this._cache.profiles[name] && !overwrite) return undefined;//profile with same name already exists
...
Profile.prototype.openKeyStore = function (buddy,vfs,password){
  ...
  return new UserFiles(files, vfs, password);
}
