...
function loadConfigFile(){
  ...
  if( fs_existsSync(config_path) ){
    ...
    if(!fs_existsSync(path.dirname(config_path))) fs.mkdirSync(path.dirname(config_path));
    config = {'profiles':{}};
...
function getProfile(name){
	...
	return ({
	   ...
	   'buddyFingerprints':function(buddy){
     	  return path.join(this.fingerprints?this.fingerprints:"",buddy?buddy:"");
	   },
       'buddies':profile_match.buddies,
       'buddyID':profile_get_buddyID,
       'addBuddy':profile_add_buddy,
       'removeBuddy':profile_remove_buddy,
       'print':profile_print,
...
function profile_remove_buddy(alias){
    var list = [];
    var latest = JSON.parse(fs.readFileSync(config_path,"utf-8"));
    latest.profiles[this.name].buddies.forEach(function(buddy){
        if(buddy.alias == alias) return;
        list.push(buddy);
    });
    latest.profiles[this.name].buddies = list;
    fs.writeFileSync(config_path,JSON.stringify(latest));
    list = [];
    config.profiles[this.name].buddies.forEach(function(buddy){
        if(buddy.alias == alias) return;
        list.push(buddy);
    });
    config.profiles[this.name].buddies = list;
}
