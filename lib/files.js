module.exports.UserFiles = UserFiles;

var fcrypto = require("./file_crypto.js");
var path = require("path");
var fs = require("fs");
var fs_existsSync = fs.existsSync || path.existsSync;
if(!path.sep){
        path.sep = (process.platform.indexOf("win")==0) ? "\\":"/";
}
function UserFiles (files, VFS, password){
    if(VFS){
      this.keys =  path_vfs(files.keys);
      this.instags =  path_vfs(files.instags);
      this.fingerprints = path_vfs(files.fingerprints);
    }else{
      this.keys =  path_real(files.keys);
      this.instags =  path_real(files.instags);
      this.fingerprints = path_real(files.fingerprints);
      make_paths(this.keys);
      make_paths(this.fingerprints);
      make_paths(this.instags);
    }

    if(VFS){
        this.VFS = VFS;
        this.encryptor = encryptor(password);

        try{
            if(this.keys) VFS.importFile(this.keys,this.keys,decryptor(password));
            if(this.instags) VFS.importFile(this.instags);
            if(this.fingerprints) VFS.importFile(this.fingerprints,this.fingerprints,decryptor(password));
        }catch(E){
            console.log("Error: Loading key-store",E);
            process.exit();
        }
    }
}

UserFiles.prototype.save = function(){
    if(this.VFS){
        console.error("Saving key-store");
        try{
          if(this.keys) this.VFS.exportFile(this.keys,this.keys,this.encryptor);
          if(this.instags) this.VFS.exportFile(this.instags);
          if(this.fingerprints) this.VFS.exportFile(this.fingerprints,this.fingerprints,this.encryptor);
        }catch(E){
            console.log("Error: Saving key-store",E);
            process.exit();
        }
    }
};

function path_real(p){
  return p?p.replace(new RegExp('/', 'g'), path.sep):p;
}
function path_vfs(p){
  return p?p.replace(new RegExp(/\\/g), '/'):p;
}
function make_paths(destination){
    if(!fs_existsSync(path_real(path.dirname(destination)))) fs.mkdirSync(path_real(path.dirname(destination)));
}

function encryptor(password){
    if(!password) return undefined;
    return (function(buff){
        return fcrypto.encryptBuffer(buff,password,"accessing key-store");
    });
}
function decryptor(password){
    if(!password) return undefined;
    return (function(buff){
        return fcrypto.decryptBuffer(buff,password,"accessing key-store");
    });
}
