...
Command.prototype.exec = function (cmdCallback) {
	...
	function selectProfileName(next) {
		var list = [];
	...
	case 'add':
		enterNewProfileInfo(function (name) {
			UI.print("Creating profile and generating your OTR key...");
			pm.createProfile(name, program.encrypt ? UI.enterNewPassword : undefined, function (err,
				profile) {
				if (err) {
					cmdCallback(err);
					return;
	...
	case 'remove':
		selectProfileName(function (err, name) {
			if (err) {
				cmdCallback(err);
				return;
			}
