...
Command.prototype.exec = function(app,profilename,id){
    ...
    if(fs_existsSync(filename)){
      ...
      if(pm.profileExists(profilename)){
       ...
       selectAccountToImport(filename,function(privkey){
         pm.createProfile(profilename,{id:id,otr:program.otr},UI.enterNewPassword,function(err,profile){
           if(err){
             console.log("Error adding new profile.",err);
             return;
           }
           ...
           profile.importKey(privkey,function(err){
             ...
             console.log("Key import complete");
             profile.print();
           });
         },true);
       });
...
function selectAccountToImport(filename,next){
