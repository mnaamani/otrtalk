...
Main.prototype.takeAction = function (action, config) {
	...
	case "chat":
		...
		main.startChatConsole(config);
		return;
	}
};

Main.prototype.startChatConsole = function (cfg) {
	var connMgr = serviceManager.require("connections-manager");
	var conn = connMgr.connectTelehash(cfg);
	connMgr.on("auth-connection", function (id) {
		//check if conn.queue has new sessions to handle..
		if (id !== conn.id) return;
		console.log("got authenticated connection");
	});
	handleCtrlC();
};
