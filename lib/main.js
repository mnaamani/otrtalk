...
Main.prototype.takeAction = function (action, config) {
	...
	case "chat":
		...
		main.startNetworkServices({
			...
			"broadcast": main._networkSettings.broadcast ? true : false,
			"torrent": main._networkSettings.torrent ? true : false,
			"enet": true,
...
Main.prototype.startNetworkServices = function (services, callback) {
	...
	socket.on("started", function () {
		if (services["broadcast"]) serviceManager.require("broadcast").start();
		if (services["telehash"]) main._startTelehashNode();
		if (services["enet"]) main._startENetNode();
		if (services["torrent"]) main._startBitTorrentNode();
...
Main.prototype._startBitTorrentNode = function () {
	serviceManager.require("torrent").start(main._networkSettings.socket);
};
