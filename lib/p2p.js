...
function enet_on_incoming(enet_peer,data){
...
function purgePeer(enet_peer,data){
    var ip = enet_peer.address().address();
    var port = enet_peer.address().port();    
    var ipp = ip+":"+port;
...
function doConnector() {
    return (function(name,onConnect,retry){
        ...
        function connect(){
          if(!active) return;
          connector.send({type:MsgType.CONNECT,snat:self.snat, ipp:self.me.ipp, localipp:localipp}, function(response){
              if( response ){
...
function doListener(name, onConnect) {
    ...
    var listener = telehash.listen(name, function ( request ) {
        if(active) handleConnect(request, onConnect);
    });
    return ({
        pause:function(){
            active=false;
...
function handleConnect(request, callback) {
  switch( parseInt(request.message.type)){
    case MsgType.CONNECT:
        if(active_peers[request.message.ipp]) return;
    ...
    case MsgType.REVERSE_READY:
       if(peers[request.message.ipp] && peers[request.message.ipp].reverse_id == request.message.id){
           if(!self.server.host.peers[request.message.ipp]){
               self.server.host.peers[request.message.ipp] = self.server.host.connect(new enet.Address(request.message.ipp), 2, parseInt(peers[request.message.ipp].reverse_id),function(err,peer){
                    if(err){
                        purgePeer(peer);
                        return;
                    }
...
function handleResponse(response, connector, callback) {
    ...
    if(parseInt(message.type) === MsgType.ACK_SNAT){
        //todo: we will need to proxy our connections through a 3rd party!
        return;
    }
    if(parseInt(message.type) === MsgType.ACK_LOCAL){
        ...
        if (!peers[message.ipp]) {            
            ...
            if(!self.server.host.peers[message.ipp]){
                ...
                self.server.host.peers[message.ipp] = self.server.host.connect(new enet.Address(message.ipp), 2, parseInt(message.id),function(err,peer){
                    if(err){
                        purgePeer(peer);
                        return;
                    }
    ...
    if(parseInt(message.type) === MsgType.ACK_REVERSE){
    ...
    if (parseInt(message.type) === MsgType.ACK){
        ...
        if (!peers[message.ipp]) {            
            ...
            if(!self.server.host.peers[message.ipp]){
                ...
                self.server.host.peers[message.ipp] = self.server.host.connect(new enet.Address(message.ipp), 2, parseInt(message.id),function(err,data){
                    if(err){
                        purgePeer(peer);
                        return;
                    }
