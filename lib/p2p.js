    ...
    return (function(name,retry){
        ...
        function connect(){
          ...
          connector.send({type:MsgType.CONNECT,snat:self.snat, ipp:self.me.ipp, localipp:localAddr}, function(response){
              if( response ){
                 handleResponse(response,connector);
              }else{        
                 //timeout..loop again
...
function handleResponse(response, connector){
    ...
    switch(parseInt(message.type)){
        ...
        case MsgType.ACK:
            popf(message.ipp);
            doOutgoingConnection(message.ipp);
            break;
...
function doOutgoingConnection(ipp){
    if(activePeers[ipp]) return;
    if(self.server.host.peers[ipp]) return;
    self.server.host.peers[ipp] = self.server.host.connect(new enet.Address(ipp), 2, 0,function(err,peer){
            if(err){
                purgePeer(peer);
            }
   }); 
