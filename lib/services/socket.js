...
SocketManager.prototype._mapPort = function (port, callback) {
	...
	if (!self._upnp) {
		self._upnp = upnp.createClient(self._settings.interface);
	}
...
function createSocket(settings, createdCallback) {
	...
	createNodeDgramSocket(port, ip, bcast, createdCallback);
}

function createNodeDgramSocket(port, ip, bcast, createdCallback) {
	createdCallback = createdCallback || function () {};
	var socket = dgram.createSocket("udp4");

	socket.on("listening", function () {
		debug("address", socket.address());
		socket.setBroadcast(bcast ? true : false);
