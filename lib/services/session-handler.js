...
function SessionHandler(peer, settings) {
	var handler = this;
	var session = settings.newSession();
	...
	session.otr.on("message", function (msg, encrypted) {
		...
		if (state.conn == ConnState.CHATTING) handler.emit('message', msg);
	});
	...
	session.otr.on("gone_secure", function () {
		...
		case 'chat':
			if (state.newFingerprint) {
				debug("not buddy fingerprint");
				handler.end();
			} else {
				if (handler.fingerprint() === settings.remote_fp && session.otr.isAuthenticated()) {
					state.conn = ConnState.AUTHENTICATED;
					handler.emit("auth");
				} else {
					handler.end();
				}
		...
		case 'connect':
			if (handler.fingerprint() !== settings.remote_fp) {
				debug("new fingerprint does not match");
				handler.end();
			}
			...
			if (state.flags.rcvd_start_smp) {
				if (state.initiator) {
					debug("initiating SMP authentication");
					session.otr.smpStart();
				}
			}
			break;
		}
	...
	session.otr.on("smp", function (type, question) {
		...
		case "request":
			...
			if (state.mode == "connect") {
				//auto SMP authentication, ignoring question...
				debug("responding to SMP authentication request");
		...
		case "complete":
			...
			if (state.mode == 'connect') {
				...
				if (state.conn == ConnState.CHATTING) handler.emit("auth-success"); //event will be consumed by chat ui
			}
			break;
		case "failed":
	...
	peer.on("message", function (packet, chan) {
		...
		if (chan === 0) {
			if (state.conn === ConnState.CHATTING || state.conn === ConnState.ENDED) return;
			try {
				control = JSON.parse(buff.toString());
				processControlMessage(control);
			} catch (E) {
				debug("connection setup error");
				peer.send(0, new Buffer(JSON.stringify({
					cmd: "ABORTING",
	...
	function processControlMessage(control) {
		...
		case "START_SMP":
			...
			if (state.mode === 'chat') {
				...
				debug("not expecting SMP authentication in chat mode");
				peer.send(0, new Buffer(JSON.stringify({
					cmd: "ABORTING",
					reason: 'NOT_EXPECTING_SMP'
				})));
				handler.end();
				break;
			} else {
		...
		case "ABORTING":
			debug("remote aborting connection:", control.reason);
			handler.end();
			break;
	...
	handler.smpRespond = function (secret) {
	...
	handler.send = function (msg) {
		if (state.conn !== ConnState.CHATTING) return;
	...
	handler.attached = function () {
		if (state.conn !== ConnState.AUTHENTICATED) return;
		state.conn = ConnState.CHATTING;
	};
	...
	handler.start = function () {
		...
		state.otrTimeout = setTimeout(function () {
			debug("timeout starting OTR conversation");
			handler.end();
		}, 30000);
	};
	...
	handler.buddy_name = function () {
		return settings.buddy_name;
	};
