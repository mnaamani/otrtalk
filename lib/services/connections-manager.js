...
ConnectionsManager.prototype.connect = function (config) {
	if (config.useBroadcast) return this.connectBroadcast(config);
...
ConnectionsManager.prototype.connectBroadcast = function (config) {
	...
	if (!bcastDiscovery) bcastDiscovery = require("./broadcast-discover").create();

	var conn = connections[connid] = {
		id: connid,
		keepLooking: true,
		config: config,
		discovery: bcastDiscovery
	};

	bcastDiscovery.on("peer", function (peer) {
		self._handleConnection(peer, connid);
	});

	bcastDiscovery.start();
...
ConnectionsManager.prototype.connectTelehash = function (config) {
	...
	if (!peerPool) {
		...
		peerPool.on("peer", function (peer, id) {
			self._handleConnection(peer, id);
		});
	}
