    ...
    p2p.server.host.on("ready", function () {
        ...
        if (arg.broadcastMode) {
            ...
            telehash.seed(function (err) {
                ...
                if (iputil.isPrivateIP(p2p.server.address().address) && (p2p.snat || arg.upnp)) {
                    didMapping = true;
		    upnp = _upnp.createClient(arg.interface);
                    upnp.portMapping({
                        ...
                        if (err) {
                            ...
                            p2p.snat = false;
                            //add to telehash (todo-speed up initial seeding)
                            if (telehash.pingSeeds) telehash.pingSeeds();
                        }
                    });
                }
