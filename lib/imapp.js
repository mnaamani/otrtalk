...
function App(app){
  this._parsed_fingerprints = [];
  var found = _.findWhere(imapps,{name:app});
...
App.prototype.supported = function(){
    return _.findWhere(this._platforms, {platform:process.platform}) ? true : false;
}

App.prototype.keystore = function(){
    var settings = _.findWhere(this._platforms, {platform:process.platform});
    if(!settings) return;
...
App.prototype.parseFingerprints = function(){
    ...
    var filename = resolve_home_path(settings.fingerprints);
      if(fs_existsSync(settings.filename)){
        ...
        if(buddies && buddies.length){
            buddies.forEach(function(line){
                ...
                if(entry[4]=='smp') this._parsed_fingerprints.push({
                    username:entry[0],
                    accountname:entry[1],
                    protocol:entry[2],
                    fingerprint:entry[3]
