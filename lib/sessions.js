                ...
                case "START_OTR":
                    ...
                    if(state.toss > parseInt(control.toss)){
                        ...
                        if(state.toss == control.toss){
                            ...
                            talk.end();
                        }else debug("[waiting for start_otr]");
                    }
                    break;
                case "START_SMP":
                    ...
                    if(!state.trust.Trusted){
                        ...
                        debug("not excpecting start_smp");
                        peer.send(0,new Buffer(JSON.stringify({cmd:"NOT_EXCPECTING_START_SMP"})));
                        talk.end();
                    }
                    break;
                case "GO_CHAT":
                    ...
                    }else if(App.mode()=='connect') console.log("\n<buddy accepted our fingerprint>");
                    break;
              }
            }catch(E){
    ...
    this.go_chat = function(){
        ...
        if(otrsession.isAuthenticated()){        
            ...
            if(state.flags.rcvd_go_chat){ talk.emit("start_chat");}
                ...
                if(App.mode()=='connect') console.log("\naccepting fingerprint, waiting for buddy.");
            }
        }
    };
    this.start = function(){
